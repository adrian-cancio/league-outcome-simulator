...
3.  **Monte Carlo Simulation (Python - `cli.py`)**:
    ...
    *   It repeatedly calls the Rust `simulate_season` function (via the Python wrapper in `simulation.py`) for a large number of iterations (configurable, e.g., 10,000 or more).
    *   In each iteration:
    ...
    *   The script aggregates the final league position achieved by each team across all simulated seasons.
...
*   **Direct Rust Bulk Simulation**: Modify `cli.py` to utilize the `simulate_bulk` function available in the Rust library. This would move the entire Monte Carlo loop into Rust, potentially offering significant performance gains by minimizing Python-Rust inter-process communication overhead.
